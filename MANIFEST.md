...
- **Shared slug seam.** `textutil.slug` is the single slug impl. `write_note` and `write_table` import it directly from the stdlib-only `textutil` (NOT from each other, so the writers can't drift and `write_table` stays PyYAML-free); `vault_mode` reuses the very same `slug` via `write_note`'s re-export (`from scripts.write_note import slug`) for its `<folder>/<slug>.md` placement — one slugging rule across all three call sites.
...
- **Untrusted folder: flag vs. enforce (defense in depth).** The classifier-supplied `frontmatter_meta["folder"]` is untrusted (an LLM may emit it over adversarial FOIA content). The two layers treat an absolute/`..`-traversal value differently *on purpose*: `taxonomy.validate_note_specs` only *flags* it (a warning keyed to the spec index — the value is never raised on or dropped), while `vault_mode._safe_relative_folder` *enforces* the hard guarantee — strip drive letters, drop empty/`.`/`..` components, fall back to the default folder if nothing survives — so no placement path can escape `out_dir`/the vault root.
